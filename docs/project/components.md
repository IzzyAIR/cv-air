...
Исключения по импорту не-типов из `src/i18n/` — шесть файлов, и все по одной причине: им
нужен список локалей или сборка пути, а это не контент.

| Файл                | Что берет   | Зачем                                  |
| ------------------- | ----------- | -------------------------------------- |
| `V2Nav.astro`       | `LANGUAGES` | Переключатель языка                     |
| `Navbar.astro`      | `LANGUAGES` | Переключатель языка в архиве            |
| `V2Layout.astro`    | `LANGUAGES` | `hreflang` по всем локалям              |
| `BaseLayout.astro`  | `LANGUAGES` | То же для `/v1`                         |
| `V2Hero.astro`      | `langHref`  | Ссылка на `/resume` текущей локали      |
| `ResumePage.astro`  | `langHref`  | Ссылка «назад на сайт»                  |
...
Клиентский JS есть в пяти файлах архива: `BaseLayout` (раскрытия и магнитная CTA), `Navbar`
(скролл-спай, мобильное меню, плавный скролл), `HeroSection` (курсорный blob),
`ExperienceSection` (аккордеон), `ContactSection` (копирование email). Перечень с деталями —
в файле `logic.md` этого раздела.

`id` секций выводятся из `navLinks[].href` (снимается ведущий `#`), поэтому ссылка в локальном
бандле и `id` в компоненте связаны жестко.
