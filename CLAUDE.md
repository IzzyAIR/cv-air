...
`src/components/ResumePage.astro` is a **standalone** light/A4 document — it does NOT use a layout. It renders its own `<html>` (white sheet, `noindex`, `<style is:inline>` instead of Tailwind), pulls the same content through the services, and ships a "Download PDF" button that calls `window.print()`. No PDF library/dependency.

**Design.** It speaks v2's language without importing it: Geist / Geist Mono (imported straight from `@fontsource-variable` in the frontmatter, so nothing loads from Google Fonts), the same cobalt accent two points darker for white paper, and the `.v2-rule` accent-gradient segment redrawn in `mm` under the header and section labels. The v2 tokens are duplicated locally as `--c-*` for the chrome only (toolbar + stage), which reads `v2-theme` from `localStorage` and follows whatever theme is pinned on the site; **the sheet is always white** — never make the paper dark.
